...
Analyze and derive. **Write all output in Brazilian Portuguese.**


1. **Breve descrição da finalidade do projeto** Avaliar qual a finalidade do projeto e escrever uma breve descrição
2. **MAPA DAS CLASSES** Mapear as units e suas classes descrevendo o caminho dos arquivos dentro do projeto.
...
Este arquivo documenta o código-fonte Delphi. Gerado por /documentar via PyDelphiAST.

<!-- token-report
 slim_ast.json   <N> chars  ~ <N/4> tokens  (canônico)
 outline.txt     <N> chars  ~ <N/4> tokens  (entrada LLM)
 DELPHI_DOCS.md  <N> chars  ~ <N/4> tokens  (este arquivo)
 Economia        ~<Nx> vs JSON bruto
-->
```

Fill in the actual values from Phase 1 output before writing.

Sections (only what the code reveals — do not invent). **All text in Brazilian Portuguese.**
